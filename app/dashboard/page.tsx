...
export default function DashboardPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    // 3. CHECK 2: If not owner, are they Staff?
    if (!businessData) {
      ...
      if (memberRecord) {
        // They are staff! Fetch the business details using the ID we found
        const { data: staffBusiness } = await supabase
          .from("businesses")
          .select("*")
          .eq("id", memberRecord.business_id)
          .single();
    ...
    // 4. RESULT: If still no business, show onboarding
    if (!businessData) {
    ...
    // 5. Fetch status for this business
    ...
    // 6. Fetch posts for this business
